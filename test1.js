...
function result(sessions) {
  return sessions.reduce((acc, curr) => {
    const sess = acc.find((item) => item.session_id === curr.session_id);
    if (!sess) {
      acc.push({
        session_id: curr.session_id,
        time: curr.time,
        classes: [{ ...curr.class, students: [curr.student] }],
      });
      return acc;
    }
    const cls = sess.classes.find(
      (item) => item.class_id === curr.class.class_id
    );
    if (!cls) {
      sess.classes.push({ ...curr.class, students: [curr.student] });
      return acc;
    }
    cls.students.push(curr.student);
    return acc;
  }, []);
}
